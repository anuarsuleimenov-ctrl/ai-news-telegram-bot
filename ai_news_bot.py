...
def send_telegram_message(text, use_html=False):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if use_html:
                ...
                print(f"[{datetime.datetime.now()}] Fallback: {'Sent!' if r2.status_code == 200 else r2.text}")
...
def inject_links(rewritten_text, articles):
    ...
    for line in lines:
        if "[LINK]" in line and article_idx < len(articles):
            ...
            article_idx += 1
        elif line.strip() == "---" and article_idx < len(articles):
            result.append(f"\u{1F517} Подробнее: {articles[article_idx]['url']}")
            result.append("")
            result.append("━━━━━━━━━━━━━━━")
            article_idx += 1
        else:
    ...
    # Add remaining links if Claude didn't add [LINK] placeholders
...
def send_daily_digest():
    ...
    articles = fetch_news()
    if not articles:
        send_telegram_message("\u{1F614} Сегодня новостей не найдено.")
        return

    today = datetime.date.today().strftime("%d.%m.%Y")
    header = f"\u{1F5DE} AI & Tech — Дайджест дня\n\u{1F4C5} {today}\n\n━━━━━━━━━━━━━━━\n\n"
    footer = "\n━━━━━━━━━━━━━━━\n\u{1F916} AI News Bot | Ежедневно в 08:00"
    ...
    if rewritten:
        ...
        lines = []
        emojis = ["\u{1F525}", "\u{26A1}", "\u{1F680}", "\u{1F916}", "\u{1F4A1}", "\u{1F9E0}", "\u{1F4CA}", "\u{1F52C}", "\u{1F4BB}", "\u{1F310}"]
        for i, a in enumerate(articles):
            emoji = emojis[i % len(emojis)]
            source = f" ({a['source']})" if a.get("source") else ""
            lines.append(f"{emoji} {a['title']}{source}\n\u{1F517} Подробнее: {a['url']}")
        msg = header + "\n\n".join(lines) + footer
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        send_telegram_message("\u{2705} AI News Bot подключён!\nClaude API: " + ("\u{2705}" if ANTHROPIC_API_KEY else "\u{274C}"))
